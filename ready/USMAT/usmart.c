...
{
    __HAL_TIM_CLEAR_FLAG(&TIM10_Handler,TIM_FLAG_UPDATE);//清除中断标志位 
    __HAL_TIM_SET_AUTORELOAD(&TIM10_Handler,0XFFFF);     //将重装载值设置到最大
    __HAL_TIM_SET_COUNTER(&TIM10_Handler,0);             //清空定时器的CNT
...
{
	if(__HAL_TIM_GET_FLAG(&TIM10_Handler,TIM_FLAG_UPDATE)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=__HAL_TIM_GET_COUNTER(&TIM10_Handler);
...
{ 		    		  			       
    if(__HAL_TIM_GET_IT_SOURCE(&TIM10_Handler,TIM_IT_UPDATE)==SET)//溢出中断
    {
        usmart_dev.scan();	//执行usmart扫描
        __HAL_TIM_SET_COUNTER(&TIM10_Handler,0);;    //清空定时器的CNT
        __HAL_TIM_SET_AUTORELOAD(&TIM10_Handler,100);//恢复原来的设置
    }
    __HAL_TIM_CLEAR_IT(&TIM10_Handler, TIM_IT_UPDATE);//清除中断标志位
}

//使能定时器4,使能中断.
void Timer10_Init(u16 arr,u16 psc)
{ 
    //定时器4
    __HAL_RCC_TIM10_CLK_ENABLE();
     
    TIM10_Handler.Instance=TIM10;                          //通用定时器4
...
{
#if USMART_ENTIMX_SCAN==1
	Timer10_Init(1000,(u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
